...
import reportRight from  '../Assets/Diabetes_Icon.png'
import reportLeft from '../Assets/Fundus Camera.png'
...
const Report = () => {
    ...
    const [fullPred,setFullPred] = useState('')
    if(!img){
        setLoading(true)
    }
    const getPrediction = async ()=>{
        setLoading(true)
        const body = {filename:img}
        console.log(body)
        try{
            const resp = await axios.post('http://localhost:8000/api/prediction/',body)
            if(resp.data.prediction[0] > resp.data.prediction[1]){
                setPred('Normal')
            }
            else{
                setPred('Disease')
            }
            setFullPred(resp.data.prediction)
        }
        catch(e){
                        ...
                        <div className='report-box'>
                           ...
                           <div>
                            ...
                            <table>
                                <tr>
                                    <td><h2>Normal</h2></td>
                                    <td><h2>{(fullPred[0]*100).toFixed(2)}%</h2></td>
                                </tr>
                                <tr>
                                    <td><h2>Disease</h2></td>
                                    <td><h2>{(fullPred[1]*100).toFixed(2)}%</h2></td>
                                </tr>
                            </table>
                           </div>
                           }
                        </div>
                        <img src = {reportLeft} style={{alignSelf:'flex-start',width:'200px'}}/>
                    </div>
                    <div>
                        <img src = {reportRight} />
                    </div>
